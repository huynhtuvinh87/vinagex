...
use backend\models\TranslationDataForm;
use yii\mongodb\Query;

class TranslationdataController extends BackendController {
...
//        }
    ...
    public function actionVi() {
        $model = new TranslationDataForm();
        $translation = (new Query)->from('translation_data')->where(['language' => 'vi'])->one();
        $dataProvider = new ArrayDataProvider([
            'allModels' => $translation['messages'],
            'pagination' => [
                'pageSize' => 10,
...
//        }
    ...
    public function actionUpdate($id) {
        $model = new TranslationDataForm(['id' => $id, 'language' => $_GET['language']]);
        $translation = (new Query)->from('translation_data')->where(['language' => $_GET['language']])->one();
        $translation_vi = (new Query)->from('translation_data')->where(['language' => 'vi'])->one();
