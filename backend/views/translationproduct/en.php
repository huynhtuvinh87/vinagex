                ...
                [
                    ...
                    'buttons' => [
                        'update' => function($url, $data, $key) {
                            return '<a href="/translationproduct/update/' . $key . '?language=en' . (!empty($_GET['page']) ? '&page=' . $_GET['page'] : '') . '"><i class="glyphicon glyphicon-pencil"></i></a>';
                        },
                    ],
    ...
    <div class="col-xs-6">
        ...
        <?php if (!empty($model->messages())) { ?>
            <div class="form-group">
                ...
                <?= Html::textArea('', $model->messages(), ['class' => 'form-control', 'readonly' => '']); ?>
            </div>
        <?php } ?>
        ...
        <?= !empty($model->id) ? '<a href="/translationproduct/' . $_GET['language'] . (!empty($_GET['page']) ? '?page=' . $_GET['page'] : '') . '" style="margin-right: 10px;" class="btn btn-default pull-right">Hủy</a>' : '' ?>
        <?php ActiveForm::end(); ?>
    </div>
</div>
<?= $this->registerJs("
